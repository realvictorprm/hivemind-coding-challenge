...
// }
case class Payload(start: LocalDate, end: LocalDate, limit: Int, minNumberReviews: Int)
...
object HelloWorld extends ZIOAppDefault {
  ...
  def app(): App[Any] =
    Http.collectZIO[Request] {
      case req@Method.GET -> Root / "amazon" / "best-rated" =>
        for {
          ...
          _ = println(payload)
          res <- Elasticsearch
            .execute(ElasticRequest.search(indexName, ElasticQuery.range(Review.unixReviewTime).gte(payload.start.toEpochDay).lt(payload.end.toEpochDay))
              .aggregate(ElasticAggregation.termsAggregation("aggr", "asin")))
            .provideLayer(Elasticsearch.layer)
            .provideLayer(ElasticExecutor.local)
            .provideLayer(HttpClientZioBackend.layer())
            .run(ZSink.collectAll)
            .map(it => Response.json(it.toVector.map(_.raw.toString()).toString()))
